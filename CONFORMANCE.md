...
## Latest reference results (all 48 gated fixtures pass, 0 pending)
...
| fad-fade | 0.929 | 0.90 | 8.80 | 0.0000 |
| fade | 0.945 | 0.90 | 8.77 | 0.0000 |
| fade-fad | 0.939 | 0.94 | 3.08 | 0.0000 |
...
| `\r`, `\rStyleName` (line-global + alignment preserved; drawing mode exits, unlike libass) | ✓ | reset tests | reset G+L | Supported |
...
5. `\fe` does not remap charsets; underline/strikeout bars are
   axis-aligned and do not follow shear or rotation (libass strokes
   them into the transformed glyph outlines).
...
7. `\r` exits drawing mode (VSFilter-compatible); libass keeps drawing
   across `\r` (verified: `{\p1}…{\r}Hi` renders drawing-continuation
   under libass 9.x, plain `Hi` here).
