...
| Misc | `Effect` field: `Banner`, `Scroll up`, `Scroll down` (timing, band clip, edge fadeaway per VSFilter); `\fe` decodes Windows single-byte plus Shift-JIS/CP949/GBK/Big5 byte-like runs before Unicode shaping; Symbol bytes use U+F000..U+F0FF | Johab (`\fe130`) and charset-based font linking stay unsupported; unknown effect names render as plain events | `HardLineBreak` exists as a tag variant but is never produced (breaks are `\n` text) |
...
- `\fe` preserves already-valid Unicode and decodes contiguous byte-like runs with replacement on invalid sequences. Symbol bytes map to the Windows Symbol private-use range and require a compatible loaded face; Johab (`\fe130`) is an explicit unsupported-codec boundary, and charset-based font linking remains unsupported by design.
