...
- Cluster-aware font fallback covers loaded faces in deterministic order (requested face → code-page-compatible faces → other faces → built-in); glyph coverage remains authoritative, and native system fonts load only via explicit `load_system_fonts()` with the `system-fonts` feature (never on WASM).
- Font collections (`.ttc`/`.otc`) load every face with per-face metadata, matching, and shaping identity (never silently face 0).
- `YCbCr Matrix` metadata is exposed at the document boundary and supports libass `Default`, `Unknown`, `None`, TV/PC 601, 709, 240m, and FCC values. Raw RGBA rendering leaves authored RGB untouched, matching libass's `ASS_Image.color` API semantics. `convert_ass_rgb` performs conversion only when the caller supplies an explicit destination `VideoColorSpace`; no video colorspace is guessed.
- `\fe` preserves already-valid Unicode and decodes contiguous byte-like runs with replacement on invalid sequences. Symbol bytes map to the Windows Symbol private-use range and require a compatible loaded face; OS/2 code-page declarations provide deterministic fallback ordering, while glyph coverage remains authoritative. Johab (`\fe130`) is an explicit unsupported-codec boundary.
