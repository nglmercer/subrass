...
| Script fields | `PlayResX/Y`, `WrapStyle`, `ScaledBorderAndShadow` (unscaled = 1:1 video px, VSFilter/legacy-libass), `LayoutResX/Y` (libass `ass_layout_res`: blur + unscaled-border denominators, unset = video size), `Kerning` (default off, like libass), `YCbCr Matrix` (`TV.*` RGB studio-range mapping; `PC.*`/`None` full-range) | | |
...
| Misc | `Effect` field: `Banner`, `Scroll up`, `Scroll down` (timing, band clip, edge fadeaway per VSFilter); `\fe` decodes Windows single-byte plus Shift-JIS/CP949/GBK/Big5 byte-like runs before Unicode shaping | Johab (`\fe130`) and Symbol font linking stay Unicode-neutral; unknown effect names render as plain events | `HardLineBreak` exists as a tag variant but is never produced (breaks are `\n` text) |
...
- `YCbCr Matrix` is applied at the RGB API boundary exactly as observed in libass: `TV.601`/`TV.709` map authored RGB to 16–235, while `PC.601`, `PC.709`, and `None` remain full-range. The 601/709 coefficient distinction requires a downstream RGB↔YCbCr video conversion, which this RGBA renderer does not perform.
- `\fe` preserves already-valid Unicode and decodes contiguous byte-like runs with replacement on invalid sequences. Johab and charset-based font linking are unsupported by design.
