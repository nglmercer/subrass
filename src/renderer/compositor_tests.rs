...
fn test_reset_keeps_drawing_mode() {
    ...
    let resolved = Compositor::resolve_style(&base, &event);
    let segments = parse_text_segments(&event.text);
    let seg = Compositor::resolve_segment_style(&resolved, &segments[1], &event, &[], 0, 0, 2000);
    assert_eq!(seg.drawing_mode, 1);
}
...
fn test_deco_underline_and_strikeout_both_paint() {
    ...
    let inside =
        (top..=bottom).any(|y| (y as usize * w..(y as usize + 1) * w).any(|i| ink[i] && !under[i]));
...
fn test_clip_libass_rect_vector_semantics() {
    ...
    let vector_only = render_event_text("{\\clip(m 320 0 l 640 0 l 640 200 l 320 200)}Hello", true);
...
fn test_positioned_anchor_is_converted_to_text_origin() {
    ...
    .unwrap();
    let resolved = Compositor::resolve_style(&style, &event);
    let (x, y) = Compositor::calculate_position(&resolved, 40.0, 20.0, 15.0, 200, 100, 200, 100);
...
fn test_transform_timing_forms() {
    ...
    let size_at = |text: &str, time_ms: u64| {
        ...
        Compositor::resolve_segment_style(&resolved, &segments[0], &event, &[], time_ms, 0, 10_000)
            .font_size
    };
...
fn test_transform_animates_supported_set() {
    ...
    let seg =
        Compositor::resolve_segment_style(&resolved, &segments[0], &event, &[], 5000, 0, 10_000);
...
fn test_fe_resolve_and_reset() {
    ...
    let resolved = Compositor::resolve_style(&base, &event);
    let segments = parse_text_segments(&event.text);
    let first = Compositor::resolve_segment_style(&resolved, &segments[0], &event, &[], 0, 0, 2000);
...
fn test_opaque_box_uses_outline_colour() {
    ...
    let buf = render_text_with_style(r"{\pos(320,400)\3c&H00FF00&}Hi", &opaque_box_style(), 1000);
